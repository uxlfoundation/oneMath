...
sycl::event gemm(backend_selector<backend::openblas> selector, transpose transa, transpose transb,
                 ...
                 std::int64_t ldc, const std::vector<sycl::event>& dependencies) {
    auto done = oneapi::math::blas::openblas::MAJOR::gemm(selector.get_queue(), transa, transb, m,
...
sycl::event hemm(backend_selector<backend::openblas> selector, side left_right, uplo upper_lower,
                 ...
                 std::int64_t ldb, std::complex<float> beta, std::complex<float>* c,
                 std::int64_t ldc, const std::vector<sycl::event>& dependencies) {
    auto done = oneapi::math::blas::openblas::MAJOR::hemm(selector.get_queue(), left_right,
